...
func TestUpdateNewNodeStatus(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			t.Cleanup(func() {
				// Cancel first so any loops watching ctx can exit.
				cancel()

				// Then stop testKubelet components and wait for them to drain.
				testKubelet.Cleanup()

				// Small quiesce to avoid racing TempDir RemoveAll (diagnostic; see §3)
...
func TestUpdateNodeStatusWithRuntimeStateError(t *testing.T) {
	...
	checkNodeStatus := func(status v1.ConditionStatus, reason string) {
		...
		updatedNode, err := kubeClient.CoreV1().Nodes().Get(tCtx, testKubeletHostname, metav1.GetOptions{})
